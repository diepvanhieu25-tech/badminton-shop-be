<aside class="w-64 hidden md:flex flex-col bg-white border-r border-slate-200">
    ...
    <div class="h-16 px-6 flex items-center gap-3 border-b border-slate-200">
        <div
            class="w-10 h-10 rounded-lg bg-emerald-600 text-white flex items-center justify-center font-bold text-lg shadow-md">
            🏸
        </div>
        <div>
    ...
    <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
        <a href="/admin"
            class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-emerald-50 transition-colors group <?= request()->is('admin') || request()->is('admin/dashboard') ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-700' ?>">
            <span class="text-lg group-hover:scale-110 transition-transform">📊</span>
            <span class="font-medium">Tổng quan</span>
        </a>

        <a href="/admin/products"
            class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-emerald-50 transition-colors group <?= request()->is('admin/products*') ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-700' ?>">
        ...
        <a href="{{ route('admin.brands.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-emerald-50 transition-colors group {{ request()->is('admin/brands*') ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-700' }}">
            <span class="text-lg group-hover:scale-110 transition-transform">🏷️</span>
        ...
        <a href="/admin/report"
            class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-emerald-50 transition-colors group <?= request()->is('admin/reports*') ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-slate-700' ?>">
            <span class="text-lg group-hover:scale-110 transition-transform">📈</span>
