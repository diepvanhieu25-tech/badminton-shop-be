...
<form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white shadow rounded-lg p-6 border border-slate-200">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2 px-3 border" placeholder="Ví dụ: Vợt cầu lông Yonex...">
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Giá bán <span class="text-red-500">*</span></label>
                        <input type="number" name="price" value="{{ old('price') }}" class="w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2 px-3 border">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Giá gốc (để gạch ngang)</label>
                        <input type="number" name="original_price" value="{{ old('original_price') }}" class="w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2 px-3 border">
                    </div>
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-slate-700 mb-1">SKU (Mã kho)</label>
                    <input type="text" name="sku" value="{{ old('sku') }}" class="w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2 px-3 border">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Mô tả chi tiết</label>
                    <textarea name="description" rows="5" class="w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm py-2 px-3 border">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-start">
                        <div class="flex h-5 items-center">
                            ...
                            <input id="is_featured" name="is_featured" value="1" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        </div>
                    ...
                    <div class="flex items-start">
                        <div class="flex h-5 items-center">
                            ...
                            <input id="has_variants" name="has_variants" value="1" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        </div>
                        <div class="ml-3 text-sm">
                            <label for="has_variants" class="font-medium text-slate-700">Có biến thể (Size/Màu)</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <a href="{{ route('admin.products.index') }}" class="block text-center w-full bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-3 px-4 rounded-lg mt-2 transition">
                Hủy bỏ
            </a>
        </div>
    </div>
